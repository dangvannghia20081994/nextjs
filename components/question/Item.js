...
const Item = ({ item, className = '', ...props }) => {
  ...
  return (
    <div className={`${className} position-relative`}>
      {item.status === 4 && (
        <span className="text-success position-absolute end-0 pe-3">
          Hỏi lần đầu
        </span>
      )}
      ...
      <div className="pt-2 d-flex">
        <div className="flex-shrink-0">
          <Image
            ...
            src={item.owner.avatar || NoAvatar}
            width={45}
            height={45}
            alt="avatar"
          />
        ...
        <div className="flex-grow-1 ms-2">
          <div className="user-name fw-bold display-html">
            {item.owner.fullName}
          </div>
          <div className="text-mute small text-secondary">
      ...
      {item.content && (
        <div
          className="content display-html pointer"
      ...
      {item.image && item.image.length > 0 ? (
        <div
          className="position-relative overflow-hidden text-center border mt-2 p-2 pointer"
          style={{ maxHeight: 300 }}
          onClick={handleViewDetail}>
      ...
      <div
        ...
        <Image
          ...
          className="pointer"
        />{" "}
        <span className="ms-1 me-2 lh-1">1</span>
        <span className="user position-relative ms-1 d-flex">
          <img
            src="https://api-dev.colearn.vn:8413/v1.0/upload/qa/image/05112021/3285eb0c-04b2-4cab-b6a1-ad84d9885ffc.jpg"
            className="avatar"
            alt="avatar"
          />
        </span>
        ...
        <div className="action ms-auto">
          <button
            className="btn btn-primary text-white py-1 px-4 rounded-4"
