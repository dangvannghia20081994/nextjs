        ...
        <div className={`${styles.form} mt-4 bg-white py-4 px-3 shadow rounded-2`}>
          <div className="row gy-4">
            <div className="col-lg-7 d-none d-lg-block">
              <Select id="custom-select-class" list={list} toggle="bg-light fw-bold rounded-4" placeholder="Chọn lớp" handerSelect={handerSelectClass} selected={selected.class} className={`select-home me-2`}/>
              <Select id="custom-select-subject" list={list} toggle="bg-light fw-bold rounded-4" placeholder="Chọn môn" handerSelect={handerSelectSubject} selected={selected.subject} className={`select-home`}/>
              <Select id="custom-select-status" list={listStatus} toggle="bg-light fw-bold rounded-4" placeholder="Trạng thái câu hỏi" handerSelect={handerSelectStatus} selected={selected.status} className={`select-home float-end`}/>
            </div>
            <div className='col-lg-5'>
              <button className={`add-question rounded-4 d-flex align-items-center bg-gradient py-2 px-4 text-white fw-bold`}>
